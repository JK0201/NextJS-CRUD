...
export default async function handler(req, res) {
  if (req.method === 'GET') {
    ...
    try {
      ...
      if (dislikeExist && !likeExist) {
        return res.status(400).json({ msg: '이미 싫어요를 눌렀습니다' });
      }
      if (!dislikeExist && !likeExist) {
        await db.collection('dislike').insertOne(likeData);
        await db.collection('posting').updateOne({ _id: postingId }, { $inc: { likes: -1 } });
        return res.status(200).json({ msg: '싫어요를 눌렀습니다' });
      } else if (!dislikeExist && likeExist) {
        await db.collection('like').deleteOne(likeData);
        await db.collection('posting').updateOne({ _id: postingId }, { $inc: { likes: -1 } });
        return res.status(200).json({ msg: '좋아요를 취소했습니다' });
      }
    } catch (err) {
